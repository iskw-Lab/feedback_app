...
import { EmotionPieChart } from "@/components/EmotionPieChart"; // 既存コンポーネントのインポート
import { ResidentRecordInfo } from "../types"; // ★types.tsから型を読み込む

interface ResidentRecordListProps {
  records: ResidentRecordInfo[];
}

export const ResidentRecordList = ({ records }: ResidentRecordListProps) => {
  return (
    <Card>
      <CardHeader>
        <CardTitle>利用者ごとの記録詳細</CardTitle>
        <CardDescription>
          選択された期間内に、あなたが各利用者について記録した件数です。各行をクリックすると詳細を表示します。
        </CardDescription>
      </CardHeader>
      <CardContent>
        {records.length > 0 ? (
              ...
              <Dialog key={item.name}>
                <DialogTrigger asChild>
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-md hover:bg-gray-100 cursor-pointer transition-colors">
                    <span className="font-medium text-gray-800">{item.name}さん</span>
                    <div className="flex items-center space-x-4">
                      ...
                      <span className="font-bold text-lg text-black">
                        {item.count} <span className="text-sm font-normal text-gray-500">件</span>
                      </span>
                        ...
                        <div className="space-y-4 max-h-80 overflow-y-auto pr-2">
                          {item.personalRecords.map((rec, i) => (
                            <div key={i} className="p-3 bg-gray-50/80 rounded-md border text-sm">
                              ...
                              <p className="mt-1 text-gray-800 font-medium">{rec.content}</p>
                            </div>
                          ))}
                        </div>
                      ) : (
          ...
          <p className="text-center text-gray-500 py-4">表示するデータがありません。</p>
        )}
      </CardContent>
    </Card>
  );
